...
parser.add_argument("-fd", "--from-dirs", dest="dirs", default=False, 
                    help="True if only tokenizing from *.ids.vids | False otherwise | default=False")
...
if __name__ == "__main__":
    ...
    if args.download is True: #preprocess without tokenizing (--for-download)
        preprocessor.parse_new()
        quit()
    elif args.dirs is True: #tokenize without preprocessing (--from-dirs)
        tokenizer = Tokenizer()
        tokenizer.format_from_dirs()
