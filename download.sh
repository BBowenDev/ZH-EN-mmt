...
function download_select {
	...
	for FILE in "$RAW"/*.ids; do
		...
		while read -r L; do
			...
			ffmpeg -ss $IN -t $LN -i $(youtube-dl $ID -q -f mp4/bestvideo --external-downloader ffmpeg "$RAW/vids/${FILE}.${ARR[0]}.mp4" || true; let ERR++) "$RAW/vids/${FILE}.${ARR[0]}.mp4" || true; let ERR++
